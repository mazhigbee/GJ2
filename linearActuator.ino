...
void loop() {
  ...
  if (positionInches < 415 && positionInches > 390 && setPosition > 375 && setPosition < 415  ) { //if we are close to center we dont want the actuator to stutter
    extending = false;
    retracting = false;
  ...
  if (extending = true && positionInches > setPosition) {
 ...
 */
  ...
  //        digitalWrite(relay1Pin, HIGH);   //this is in
  //      }
  ...
  //      if (button1State == LOW && button2State == HIGH) {     //this is out
  //        // turn relay2 on:
  //        //positionInches = positionInches + copyCount;
  //        digitalWrite(relay2Pin, HIGH);
  //      }
